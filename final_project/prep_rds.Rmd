...
```
```{r}
remove_words <- c("january", "february", "march", "april", "may", "june","july","august","september","october","november","december","jan", "feb", "mar", "apr", "may", "jun", "jul", "aug", "sep", "oct", "nov", "dec","q1", "q2", "q3", "q4","policy", "economic", "monetary", "thai", "term", "rate","growth")
...
top_words_q <- tokens %>%  
  ...
  filter(!(word %in% remove_words)) %>%       # Remove specified words
  filter(!str_detect(word, "^[0-9]+(\\.[0-9]+)?$")) %>%  # Remove numbers
  count(word, year, quarter, sort = TRUE) %>% 
  mutate(year_quarter = paste0(year, "-", quarter)) %>%
...
top_words_q_select <-  top_words_q %>% 
  filter(year_quarter == '2020-Q1') %>%                # Count word frequencies
  slice_head(n = 25) 
...
plot2_q <- ggplot(top_words_q_select, aes(y = reorder(word, n), x = n))  +
  ...
  labs(
    x = "Count", y = NULL,
    title = "<span style='font-size:14pt;'><b>Most common words in Monetary Policy Report: 2020-Q1</b></span>") +
  # Theme settings
  theme_minimal() +   
  theme(
    plot.title = element_markdown(hjust = 0.5),  # Centered title with HTML styling
...
plot2_word <- ggplot(top_words_word_select, aes(x = year_quarter, y = ngram_percentage, color = word, group = word)) +
  geom_line(size = 1) +  # Line graph
  geom_point(size = 2) +  # Add points for emphasis
  labs(
    x = "Year-Quarter", y = "Percentage",
